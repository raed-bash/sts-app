...
Environment variables are validated at startup by a Zod schema in `src/config/env.ts` — invalid or missing values fail fast with a readable error.
...
A Husky `pre-commit` hook runs `npm run validate` before every commit.
...
- **Authentication** — the Axios instance in `src/lib/api.ts` injects the bearer token from `localStorage` on every request via a request interceptor, so a cold start with a stored session works without an explicit `setAuthToken` call.
...
- **Storybook** documents the shared components (`src/shared/components/**/*.stories.tsx`).
- **Playwright** covers the critical auth and home flows in `e2e/`.

```bash
npm run validate   # everything the pre-commit hook enforces
```
